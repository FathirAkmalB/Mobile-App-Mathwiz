...
import 'package:consume_api/UI/remedial_page/remedial_screens.dart';
import 'package:consume_api/UI/remedial_page/result_remedial.dart';
...
class ListRemedialScreen extends StatefulWidget {
  final subjectExam;
  final studentId;
  const ListRemedialScreen(
      {super.key, this.subjectExam, this.studentId});

  @override
  State<ListRemedialScreen> createState() => _ListRemedialScreenState();
}

class _ListRemedialScreenState extends State<ListRemedialScreen> {
  int _currentIndex = 2;
  ...
  // Fetch List of Remedial Data
  Future<Map> getRemedialBySubject({String? filter}) async {
    final response = await API().getRequest(
        route:
            '/getRemedialBySubject/${widget.subjectExam['id']}/${widget.studentId}');
    if (response.statusCode == 200) {
      final data = json.decode(response.body);
      final totalRemedial = data['total_remedial'];
      final totalRemedialHasAttempted = data['total_attempted_remedial'];
      ...
      if (filter != null && filter.isNotEmpty) {
        ...
        return {
          'totalRemedial': lengthData,
          'dataExam': filteredData,
          'totalRemedialHasAttempted': totalRemedialHasAttempted,
          'studentId': widget.studentId,
        };
      } else {
        return {
          'dataExam': dataExam,
          'totalRemedial': totalRemedial,
          'totalRemedialHasAttempted': totalRemedialHasAttempted,
        };
      }
    } else {
                      ...
                      child: TextField(
                        onChanged: (value) {
                          setState(() {
                            _searchQuery = value;
                            print(_searchQuery);
                            getRemedialBySubject(filter: _searchQuery);
                          });
                        },
                        ...
                        FutureBuilder(
                          ...
                          builder: (context, snapshot) {
                            if (snapshot.connectionState ==
                              ...
                              final totalRemedial = snapshot.data!['totalRemedial'];
                              return Text(
                      ...
                      child: FutureBuilder(
                          ...
                          builder: (context, snapshot) {
                            if (snapshot.connectionState ==
                              ...
                              final totalRemedial = data['totalRemedial'];
                                                      ...
                                                      onTap: !hasAttempt
                                                          ...
                                                          : () {
                                                              Navigator.push(
                                                                context,
                                                                MaterialPageRoute(
                                                                  builder: (context) => ResultRemedial(
                                                                      examID:
                                                                          exam['id'],
                                                                      studentId:
                                                                          widget
  ...
  Future<void> _refreshData() async {
    setState(() {
      exerciseList = getRemedialBySubject();
    });
  }
}
