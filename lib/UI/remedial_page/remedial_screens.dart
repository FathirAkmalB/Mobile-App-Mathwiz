...
class RemedialScreen extends StatefulWidget {
  final studentId;
  final exam;
  const RemedialScreen({super.key, required this.exam, required this.studentId});

  @override
  State<RemedialScreen> createState() => _RemedialScreen();
}

class _RemedialScreen extends State<RemedialScreen> {
  initState() {
    super.initState();
  ...
  Future<Map> checkAttempt() async {
    final response = await API().getRequest(route: '/checkAttemptForRemedial/${widget.exam['id']}/${widget.studentId}');
  ...
  Future<void> postRemedial() async {
    ...
    final response = await API()
        .postRequest(route: '/takeRemedial/${widget.exam['id']}', data: data);
   ...
   // ignore: use_build_context_synchronously
    Navigator.push(
        ...
        MaterialPageRoute(
          builder: (context) =>
              ResultRemedial(examID: widget.exam['id'], studentId: studentId),
        ));
    } else {
